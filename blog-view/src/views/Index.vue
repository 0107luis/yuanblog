  ...
  <div class="index-container">
    ...
    <div class="index-main" id="index" >
      ...
      <div :class="{'m-display-none':focusMode}" class="index-main-column-left">
        <Card></Card>
        <Calendar></Calendar>
        <CardBlog cardHeader="最新博客" :blogs="newBlogs"></CardBlog>
      </div>
      <!--中间-->
      ...
      <div :class="{'m-display-none':focusMode}" class="index-main-column-right">
        ...
        <CardBlog cardHeader="最热博客" :blogs="hotBlogs"></CardBlog>
        <CardBlog cardHeader="本周热议" :blogs="weekBotBlogs"></CardBlog>
      </div>
    </div>
    <!--回到顶部-->
...
export default {
  name: "Index",
  components: {RightCard, Footer, Navbar, Card, Calendar,HeadPhoto, CardTag, CardBlog},
  data() {
    return {
      focusMode: false,
      hotTags: [],
...
//获取最热标签
    getHotTags() {
      let that = this
      this.$axios.get('/tags/hot').then(res => {
...
// 获取最热博客
    getHotBlogs() {
      ...
      this.$axios.get('/blog/hot').then(res => {
        that.hotBlogs = res.data.data
      }).catch(error => {
        console.error('获取最热博客失败:', error)
        // 设置默认空数组，避免页面报错
        that.hotBlogs = []
        if (error !== 'error') {
          that.$message({type: 'error', message: '最热博客加载失败!', showClose: true})
        }
      })
    },
// 获取最新博客
    getNewBlogs() {
      ...
      this.$axios.get('/blog/new').then(res => {
        ...
        that.newBlogs = []
        if (error !== 'error') {
          that.$message({type: 'error', message: '最新博客加载失败!', showClose: true})
        }
      })
    },
// 获取最新博客
    getWeekHotBlogs() {
      let that = this
      this.$axios.get('/blog/weekhot').then(res => {
        that.weekBotBlogs = res.data.data
      }).catch(error => {
        console.error('获取本周热议博客失败:', error)
        // 设置默认空数组，避免页面报错
        that.weekBotBlogs = []
        if (error !== 'error') {
          that.$message({type: 'error', message: '本周热议博客加载失败!', showClose: true})
        }
      })
    },
  },
...
.index-container::before {
  ...
  right: 0;
  bottom: 0;
  background-image: url("http://r9vd4j8w3.hd-bkt.clouddn.com/img/background2.png");
